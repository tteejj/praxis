...
class MinimalListBox : FocusableComponent {
    ...
    [string] RenderContent() {
        ...
        # Draw items - ensure we start at least at Y=1 to avoid terminal edge issues
        $startY = [Math]::Max(1, $this.Y)
        ...
        for ($i = $this._scrollOffset; $i -lt $endIndex; $i++) {
            ...
            $availableWidth = $this.Width - 2  # -2 for prefix
            if ($this.ShowBorder) { $availableWidth -= 2 }  # Additional -2 for border
            ...
            # Use RenderHelper for consistent list item rendering
            $isSelected = ($i -eq $this.SelectedIndex)
            $sb.Append([RenderHelper]::RenderListItem($text, $isSelected, $this.IsFocused, $availableWidth, $this.Theme))
