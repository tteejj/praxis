...
class DestructiveActionDialog : BaseDialog {
    ...
    [void] OnActivated() {
        ...
        # Focus on cancel button by default (safer for destructive actions)
        $focusManager = $this.ServiceContainer.GetService('FocusManager')
        if ($focusManager -and $this.PrimaryButton) {
            $focusManager.SetFocus($this.PrimaryButton)
        } elseif ($this.PrimaryButton) {
