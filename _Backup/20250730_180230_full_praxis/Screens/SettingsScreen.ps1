...
class SettingsScreen : Screen {
    ...
    [bool] HandleScreenInput([System.ConsoleKeyInfo]$key) {
        ...
        switch ($key.Key) {
            ...
            ([System.ConsoleKey]::Tab) {
                # Toggle focus between CategoryList and SettingsGrid using FocusManager
                $focusManager = $this.ServiceContainer.GetService('FocusManager')
                if ($focusManager) {
                    if ($this.CategoryList.IsFocused) {
                        if ($this.SettingsGrid.Items.Count -gt 0) {
                            $focusManager.SetFocus($this.SettingsGrid)
                        }
                    } else {
                        $focusManager.SetFocus($this.CategoryList)
                    }
                }
                return $true
            }
            ([System.ConsoleKey]::LeftArrow) {
                # Focus category list using FocusManager
                $focusManager = $this.ServiceContainer.GetService('FocusManager')
                if ($focusManager) {
                    $focusManager.SetFocus($this.CategoryList)
                }
            ...
            ([System.ConsoleKey]::RightArrow) {
                # Focus settings grid if it has items using FocusManager
                $focusManager = $this.ServiceContainer.GetService('FocusManager')
                if ($focusManager -and $this.SettingsGrid.Items.Count -gt 0) {
                    $focusManager.SetFocus($this.SettingsGrid)
                }
                return $true
            }
    ...
    [void] EditSetting() {
        ...
        # If CategoryList has focus, move focus to SettingsGrid first using FocusManager
