...
class ToastService {
    ...
    [void] RenderToast([System.Text.StringBuilder]$sb, [Toast]$toast, [int]$x, [int]$y) {
        ...
        $colors = switch ($toast.Type) {
            ([ToastType]::Success) { @{
                bg = $this.Theme.GetBgColor('status.success')
                fg = $this.Theme.GetColor('text.primary')
                icon = "✓"
            }}
            ([ToastType]::Warning) { @{
                bg = $this.Theme.GetBgColor('status.warning')
                fg = $this.Theme.GetColor('text.primary')
                icon = "!"
            }}
            ([ToastType]::Error) { @{
                bg = $this.Theme.GetBgColor('status.error')
                fg = $this.Theme.GetColor('text.primary')
                icon = "✗"
            }}
            default { @{
                bg = $this.Theme.GetBgColor('status.info')
                fg = $this.Theme.GetColor('text.primary')
                icon = "i"
            }}
        }
