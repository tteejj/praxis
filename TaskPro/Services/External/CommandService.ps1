# CommandService.ps1 - Service for managing command library with unified data persistence
...
class CommandService {
    ...
    CommandService() {
        ...
        $this.BackupDirectory = Join-Path $praxisRoot "_ProjectData/backups"
    ...
    # Load commands from unified data
    [void] LoadCommands() {
        try {
            # Load commands from unified data service
            $data = [PraxisDataService]::GetCommands()
            $this.Commands.Clear()
            
            foreach ($commandData in $data) {
            ...
            if ($global:Debug) {
                Write-Host "Loaded $($this.Commands.Count) commands from unified data" -ForegroundColor Green
            }
            
            # If no commands, create default ones
            if ($this.Commands.Count -eq 0) {
                Write-Host "No existing commands found, creating default commands" -ForegroundColor Yellow
                $this.CreateDefaultCommands()
            }
        } catch {
            Write-Host "Failed to load commands from unified data: $($_.Exception.Message)" -ForegroundColor Red
            # Create default commands on error
            $this.Commands.Clear()
            $this.CreateDefaultCommands()
        }
    }
    ...
    # Save commands to unified data service
    [void] SaveCommands() {
        try {
            $hashtables = $this.Commands | ForEach-Object { $_.ToHashtable() }
            
            # Save to DataPool if adapter is available
            if ($this.DataAdapter) {
                $this.DataAdapter.SaveCommands($hashtables)
            } else {
                # Save to unified data service
                [PraxisDataService]::UpdateCommands($hashtables)
            }
            
            if ($global:Debug) {
                Write-Host "Commands saved successfully to unified data" -ForegroundColor Green
            }
        } catch {
            Write-Host "Failed to save commands to unified data: $($_.Exception.Message)" -ForegroundColor Red
            throw
        }
    }
    ...
    [void] CopyToClipboard([string]$commandId) {
        $this.CopyToClipboardWithRecent($commandId)
    }
    ...
    [void] CheckForIncomingData() {
        if ($this.DataAdapter) {
            $exchanges = $this.DataAdapter.CheckExchanges()
            foreach ($exchange in $exchanges) {
                # Process time data from TimeTracker
                $commandData = $this.DataAdapter.ProcessTimeDataExchange($exchange)
                if ($commandData) {
                    $this.AddCommand(
                        $commandData.Title,
                    ...
                    Write-Host "Created command from TimeTracker data: $($commandData.Title)" -ForegroundColor Green
                }
            }
            
            # Check for Excel imports
            $excelImports = $this.DataAdapter.ImportFromExcel()
            foreach ($importData in $excelImports) {
                # Convert import data to command
                $command = [Command]::FromHashtable($importData)
                $this.Commands.Add($command) | Out-Null
                Write-Host "Imported command from Excel: $($command.Title)" -ForegroundColor Green
            }
            
            if ($excelImports.Count -gt 0) {
                $this.SaveCommands()
            }
        }
    }
    ...
    [void] SendToMacroFactory([Command]$command) {
        if ($this.DataAdapter) {
            $this.DataAdapter.SendToMacroFactory($command)
            Write-Host "Command sent to MacroFactory for visual editing" -ForegroundColor Green
        }
    }
    ...
    [void] ExportToExcel() {
        if ($this.DataAdapter) {
            $this.DataAdapter.ExportToExcel($this.Commands.ToArray())
            Write-Host "Commands exported to ExcelDataFlow" -ForegroundColor Green
        }
    }
    
    # Override methods to add to recent items
    [void] CopyToClipboardWithRecent([string]$commandId) {
        $command = $this.GetCommandById($commandId)
        if ($command) {
            ...
            try {
                # Simple approach - just use Set-Clipboard and let it fail gracefully
                $command.CommandText | Set-Clipboard -ErrorAction Stop
                $success = $true
            } catch {
                # If Set-Clipboard fails (like on Linux), show a helpful message
                $errorMsg = "Clipboard not available - $($_.Exception.Message)"
            }
            
            if (-not $success -and -not $errorMsg) {
                $errorMsg = "Clipboard operation failed"
            }
            
            # Always record usage even if clipboard fails
            $command.RecordUsage()
            $this.SaveCommands()
            
            # Add to recent items
            if ($this.DataAdapter) {
                $this.DataAdapter.AddToRecent($command)
            }
            
            # Throw error if clipboard failed so caller can handle it
            if (-not $success) {
                throw $errorMsg
            }
        }
