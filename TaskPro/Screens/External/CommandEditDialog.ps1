...
class CommandEditDialog : SimpleDialog {
    ...
    [void] SaveCommand() {
        ...
        # Validate that we have either a title or command text
        if ([string]::IsNullOrWhiteSpace($commandText) -and [string]::IsNullOrWhiteSpace($title)) {
            # Show error and return without saving
            [Console]::Clear()
            Write-Host ""
            Write-Host "ERROR: Command must have either a title or command text!" -ForegroundColor Red
            Write-Host ""
        ...
        if ([string]::IsNullOrWhiteSpace($commandText)) {
            # If no command text, show warning but allow save
            [Console]::Clear()
            Write-Host ""
            Write-Host "WARNING: No command text specified. This command will not be executable." -ForegroundColor Yellow
            Write-Host ""
            Write-Host "Continue saving? (Y/n): " -NoNewline -ForegroundColor Gray
            $response = Read-Host
