...
class ThemeEditorDialog : BaseDialog {
        ...
        foreach ($key in $theme.Keys | Sort-Object) {
            if ($theme[$key] -is [array] -and $theme[$key].Count -eq 3) {
                $item = @{
                    Key = $key
                    RGB = $theme[$key]
                    IsSemantic = @{SemanticTokens=@{}}::SemanticTokens.ContainsKey($key)
                }
    ...
    [void] ApplyColorChange() {
        ...
        # Apply to live theme
        if ($this.ThemeManager -is [ThemeManager]) {
    ...
    [string] OnRender() {
        ...
        if ($this.SelectedColorKey -and @{SemanticTokens=@{}}::SemanticTokens.ContainsKey($this.SelectedColorKey)) {
