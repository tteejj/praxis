...
class FullNotesEditor {
    ...
    [void] CreateBackup([string]$content) {
        try {
            ...
            $backupFile = Join-Path $this.BackupDirectory "notes_backup_$timestamp.txt"
            ...
            $backups = Get-ChildItem -Path $this.BackupDirectory -Filter "notes_backup_*.txt" | 
    ...
    [void] AutoSaveIfNeeded() {
        if ($this.HasUnsavedChanges()) {
            ...
            $autoSaveFile = Join-Path $this.BackupDirectory "autosave_notes.txt"
